...
function LiveStreamModal({ open, onClose, user }) {
  ...
  return /* @__PURE__ */ jsx("div", { className: `fixed inset-0 z-50 bg-black/60 flex items-start justify-center overflow-y-auto overscroll-contain ${isFullscreen ? "p-0" : "p-4 pb-28"}`, children: /* @__PURE__ */ jsxs("div", { ref: modalRef, className: `${isFullscreen ? "w-screen h-screen max-w-none rounded-none flex flex-col" : "w-full max-w-3xl max-h-[calc(100vh-7rem)] rounded-3xl flex flex-col"} bg-white shadow-2xl overflow-hidden relative my-auto`, children: [
    /* Header */
    ...
    /* Body */
    /* @__PURE__ */ jsxs("div", { className: `${isFullscreen ? "p-4 pr-48 sm:pr-60 grid grid-cols-1 lg:grid-cols-2 gap-4 flex-1 overflow-y-auto overscroll-contain pb-24" : "p-4 grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 overflow-y-auto overscroll-contain pb-28"}`, children: [
      /* Left: video */
      /* @__PURE__ */ jsxs("div", { className: "space-y-3", children: [
...
const PachinkoView = ({ user, profile, onBack, showNotification }) => {
  // v13: form inputs are rendered as stable elements so typing does not lose focus on each character.
  ...
  const drawCard = () => {
    ...
    return { suit: cardSuits[Math.floor(Math.random() * cardSuits.length)], rank, value, rankIndex };
  };
  ...
  const bjCountText = (cards, hideSecond = false) => {
    if (!cards || cards.length === 0) return "0";
    if (hideSecond) return `${bjCardValue(cards[0])} + ?`;
    const raw = cards.map((c) => c?.rank === "A" ? "A(11/1)" : String(bjCardValue(c))).join(" + ");
    return `${raw} = ${bjTotal(cards)}`;
  };
  const parseBet = () => {
  ...
  const CardChip = ({ card, hidden = false }) => /* @__PURE__ */ jsx("div", { className: `min-w-14 h-20 rounded-2xl border shadow-lg flex items-center justify-center text-xl font-black bg-white ${hidden ? "text-gray-300 bg-gray-900" : cardColor(card)}`, children: hidden ? "?" : /* @__PURE__ */ jsxs("div", { className: "text-center leading-tight", children: [
    /* @__PURE__ */ jsx("div", { children: cardText(card) }),
    card && /* @__PURE__ */ jsx("div", { className: "text-[10px] font-black text-gray-400 mt-1", children: card.rank === "A" ? "11/1" : bjCardValue(card) })
  ] }) });
  const DiceChip = ({ d, active = false }) => /* @__PURE__ */ jsx("div", { className: `w-20 h-20 rounded-3xl bg-white shadow-xl border flex items-center justify-center text-4xl font-black text-gray-900 ${active ? "animate-bounce" : ""}`, children: d });
  const ResultBox = () => lastResult && /* @__PURE__ */ jsxs("div", { className: "mt-4 bg-white border rounded-3xl p-4 shadow-sm", children: [
    ...
    /* @__PURE__ */ jsx("div", { className: "mt-1 text-sm font-black text-gray-950 break-words", children: lastResult.resultText || "結果なし" }),
  ...
  if (page === "blackjack") return /* @__PURE__ */ jsxs("div", { className: "w-full h-full flex flex-col bg-slate-100", children: [
    ...
    /* @__PURE__ */ jsxs("div", { className: "flex-1 overflow-y-auto p-4", children: [
      ...
      /* @__PURE__ */ jsxs("div", { className: "rounded-[36px] bg-gradient-to-br from-gray-950 to-slate-700 p-6 shadow-2xl text-white", children: [
        ...
        /* @__PURE__ */ jsxs("div", { className: "mb-5", children: [
          /* @__PURE__ */ jsx("div", { className: "text-xs font-black text-gray-300 mb-2", children: `YOU ${bjCountText(blackjack.player)}` }),
